...
activity = activity_csv.loc[:, ['MonitoringLocationIdentifier', 
                 'ActivityIdentifier',
                 'ActivityTypeCode',
                 'ActivityMediaName',
                 'ActivityStartDate',
...
# %%
# Rename Columns
activity.columns = ['station_id', 'activity_id', 
                    'activity_type', 'activity_media',
                    'activity_start_date', 'activity_start_time', 
                    'activity_time_zone', 'activity_depth_name', 
                    'activity_depth', 'activity_depth_unit', 
                    'activity_comment']
...
activity['activity_start_time'] = pd.to_datetime(activity['activity_start_time'], 
                                                 format = '%H:%M:%S')
#%%                                                 errors = 'coerce')
...
object_cols = ['station_id', "activity_id", 'activity_type',
               'activity_media', 'activity_time_zone', 'activity_depth_name',
               'activity_depth_unit', 'activity_comment']
...
print(activity.apply(lambda col: col.isin([".", "-", "n/a", "NA", "Not determined", ""]).any()))
...
# %%
# Select the columns of interest for the database
station = station_csv.loc[:, ['MonitoringLocationIdentifier', 
...
# %%
# Rename Columns
station.columns = ['station_id', 'station_name', 
                    'station_type', 'latitude',
                    'longitude', 'huc8', 
...
#%%
# Save all other columns as strings
station_object_dtype = ['station_id', 'station_name', 'station_type',
               'provider_name']
...
# %%
# Check for fake NA values
print(station.apply(lambda col: col.isin([".", "-", "n/a", "NA", "Not determined", ""]).any()))
...
# %%
# Select the columns of interest for the database
result = result_csv.loc[:, ['ActivityIdentifier', 
                 'ProjectIdentifier',
...
# %%
# Rename Columns
result.columns = ['activity_id', 'project_id', 
                    'result_condition', 'characteristic_name',
                    'result_value', 'result_unit', 
                    'result_status', 'result_type',
                    'analytical_method_id', 'analytical_method',
                    'lab_name', 'detection_limit_type',
                    'detection_limit_value', 'detection_limit_unit']
...
#%%
# Save all other columns as strings
result_float_dtype = ['result_value', 'detection_limit_value', 'station_type',
               'provider_name']
